...
The door opener is expected to listen to open messages, defining their topic like
...
`--cient-suffix <suffix>`::
By default, each instance appends an UUID to the MQTT client ID. This allows testing without affecting running agents (would get disconnected if using the same ID). With this option, a well-defined suffix can be forced, to create a recognizable and unique instance.
